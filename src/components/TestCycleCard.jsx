                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                            <span className="text-[10px] font-bold text-slate-400 flex items-center gap-1 uppercase">
                                <Building2 className="w-3 h-3" /> Soc: {cycle.company_code || '1000'}
                            </span>
                            <span className="text-[10px] text-slate-200">|</span>
                            <span className="text-[10px] font-bold text-slate-400 flex items-center gap-1 uppercase">
                                <Globe className="w-3 h-3" /> {cycle.purchase_type || 'Nacional'}
                            </span>
                        </div>
                    </div>
                    ...
                    <div className="mb-6 p-4 bg-slate-50 rounded-xl border border-slate-200 fade-in shadow-inner">
                        ...
                        <div className="grid grid-cols-2 gap-x-4 gap-y-3">
                            <div className="flex flex-col">
                                ...
                                <span className="text-[11px] font-bold text-slate-800">{cycle.vendor || 'N/A'}</span>
                            </div>
                            <div className="flex flex-col">
                                <span className="text-[9px] text-slate-400 font-bold uppercase">Cliente</span>
                                <span className="text-[11px] font-bold text-slate-800">{cycle.client || 'N/A'}</span>
                            </div>
                            <div className="flex flex-col">
                                <span className="text-[9px] text-slate-400 font-bold uppercase">Material</span>
                                <span className="text-[11px] font-bold text-slate-800">{cycle.material || 'N/A'}</span>
                            </div>
                            <div className="flex flex-col">
                                <span className="text-[9px] text-slate-400 font-bold uppercase">Sociedad</span>
                                <span className="text-[11px] font-bold text-slate-800">{cycle.company_code || '1000'}</span>
                            </div>
                        </div>
                    ...
                    {!canEdit && !isCompleted && (
                        <div className="p-4 bg-slate-50 rounded-2xl flex items-center gap-4 border border-slate-100">
                            ...
                            <p className="text-xs font-semibold text-slate-500 italic">
                                Acceso restringido. Solo el área de {currentStepData?.team} puede realizar este registro.
                            </p>
                        </div>
                    )}
                                    ...
                                    <div key={step.index} className="flex flex-col group">
                                        <div className="flex items-center justify-between text-xs">
                                            ...
                                            <span className={`font-mono font-bold px-2 py-1 rounded ${doc ? 'text-blue-900 bg-blue-50 border border-blue-100' : 'text-slate-300'
                                                ...
                                                {isMetadata ? (doc.value || 'OK') : (doc || 'POR REGISTRAR')}
                                            </span>
                                        </div>
                                        ...
                                        {isMetadata && (
                                            <div className="mt-2 ml-3.5 pl-3 border-l-2 border-slate-100 flex flex-col gap-0.5">
                                                <p className="text-[10px] text-slate-500 font-bold">
                                                    Registrado por: <span className="text-blue-900">{doc.user || 'N/A'}</span>
                                                </p>
                                                <div className="flex items-center gap-2 text-[9px] text-slate-400 font-medium">
                                                    <span className="bg-slate-100 px-1.5 py-0.5 rounded uppercase">{doc.area || 'Sistema'}</span>
                                                    <span>•</span>
